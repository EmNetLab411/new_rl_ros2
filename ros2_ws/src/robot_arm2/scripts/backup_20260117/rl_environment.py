...
# Original full workspace commented out for reference:
#   SURFACE_X_MIN = -0.24, SURFACE_X_MAX = 0.24
#   SURFACE_Y_MIN = -0.35, SURFACE_Y_MAX = -0.05
#   SURFACE_Z_MIN = 0.08,  SURFACE_Z_MAX = 0.40

SURFACE_X_MIN = -0.06  # -6cm
SURFACE_X_MAX = 0.06   # +6cm  → 12cm width (X)
SURFACE_Y_MIN = -0.30  # -30cm
SURFACE_Y_MAX = -0.15  # -15cm → 15cm depth (Y)
...
class RLEnvironment(Node):
    ...
    def _calculate_reward(self, dist_after: float, dist_before: float) -> Tuple[float, bool]:
        ...
        if dist_after < self.goal_tolerance:  # 0.01m = 1cm
            ...
            # ----- FIXED REWARD (addresses all 4 analysis issues) -----
            
            # Distance penalty
            dist_reward = -5.0 * dist_after
            
            # Improvement: SYMMETRIC (no 2.5× asymmetric - fixes gradient issue)
            improvement = dist_before - dist_after
            improve_reward = 10.0 * improvement
            
            # Step penalty: REDUCED (was 0.5, now 0.1 - fixes 10:1 ratio)
            step_penalty = 0.1
            
            # Combine (NO CLIP - fixes gradient loss)
