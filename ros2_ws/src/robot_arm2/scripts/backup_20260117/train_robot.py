...
# Reward settings (must match rl_environment.py _calculate_reward)
# OLD Formula: -5×d + asymmetric_improve - 0.5, success=+10, clip=±10
GOAL_THRESHOLD = 0.01  # 1cm threshold for success
SUCCESS_REWARD = 10.0  # OLD-style balanced success signal
STEP_PENALTY = -0.5  # OLD-style step penalty (creates urgency)

# TD3 hyperparameters
ACTOR_LR = 3e-4
CRITIC_LR = 1e-4  # Reduced from 3e-4 to stabilize Q-value estimates
GAMMA = 0.99
TAU = 0.005
BATCH_SIZE = 256
BUFFER_SIZE = int(1e6)
        ...
        for episode in range(args.episodes):
            ...
            for step in range(args.max_steps):
                ...
                if next_state is not None and len(next_state) >= 12:
                    ...
                    if done and reward > 5.0:  # Success reward is +10 (OLD formula)
                ...
                if done and reward > 5.0:  # Goal reached with success bonus (OLD: +10)
